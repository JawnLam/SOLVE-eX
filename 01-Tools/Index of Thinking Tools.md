...
Entries use the `Thinking_Tool` prototype (Master_Schema.yaml v1.13.0+). The `tt_` namespace anchors the classification facets; `Universal_Core` provides identity (`Item_ID`, `Item_Prototype`, `Title`) and admin fields. `tt_Domain` is single-valued (one canonical register); `tt_Cross_Domains` lists additional register spans. 9 cross-cutting facets in v1.13: Type, Form, Scale, Duration, Lineage, Posture, **State**, **Agent**, **About** *(last three added in v1.13)*.
